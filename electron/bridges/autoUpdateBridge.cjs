 ...
 * getAllWindows()[0]) so we never query settings windows, whose renderers
 ...
 * permanently bypass the dirty-editor quit guard. This watchdog clears them
 * if we're still running after a grace period.
 *
 ...
 * close handler and get stranded again — the exact #1215 failure. So we only
 * roll back after a window long enough that the app is realistically stuck,
 * not merely slow. The cost of waiting longer is just that isQuitting stays
...
function registerHandlers(ipcMain) {
  ...
  ipcMain.handle("netcatty:update:install", async () => {
    ...
    // actually close, the before-quit dirty-editor guard can run after the
    // window is already gone — isReachableByUser is false, so it commits the
    // quit and silently drops unsaved SFTP edits.
    //
    ...
    // hotkey), which prevents quitAndInstall from completing. Unregister the
    // global hotkey and clear the Dock menu before quitting so the app can
    // exit cleanly and the installer can proceed.
    try {
    ...
    // every window close bypasses the dirty-editor guard.
