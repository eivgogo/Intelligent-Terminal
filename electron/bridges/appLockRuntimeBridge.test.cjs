...
async function createControllerHarness(options = {}) {
  ...
  const controller = createAppLockController({
    ...
    waitForUnlockFailureDelay:
      options.waitForUnlockFailureDelay ??
      (async (delayMs) => {
...
test("system unlock setting cannot be disabled without password while locked", async () => {
  ...
  const saved = await controller.setSystemUnlockEnabled({
    enabled: false,
...
test("system unlock result is rejected when the setting is disabled during verification", async () => {
  ...
  systemAuthBridge.requestUnlock = async () =>
    new Promise((resolve) => {
...
test("system unlock succeeds only when enabled and locked", async () => {
  const { controller, runtimeBridge, systemAuthCalls } =
    await createControllerHarness();
  await controller.requestPasswordChange({ nextPassword: "alpha" });
  await controller.requestEnable();

  assert.deepEqual(await controller.requestSystemUnlock(), {
    ok: false,
    error: "disabled",
  });
  await controller.setSystemUnlockEnabled({ enabled: true });
...
test("system unlock cancellation preserves locked runtime state", async () => {
  const { controller, runtimeBridge, systemAuthBridge } =
    await createControllerHarness();
  await controller.requestPasswordChange({ nextPassword: "alpha" });
  await controller.requestEnable();
  await controller.setSystemUnlockEnabled({
    enabled: true,
    currentPassword: "alpha",
  });
  controller.setLocked("manual");
...
test("system unlock IPC handlers are registered", async () => {
  ...
  assert.equal(
    ipcMain.handlers.has("netcatty:appLock:getSystemUnlockStatus"),
    true,
  );
  assert.equal(
    ipcMain.handlers.has("netcatty:appLock:setSystemUnlockEnabled"),
    true,
  );
  assert.equal(
    ipcMain.handlers.has("netcatty:appLock:requestSystemUnlock"),
    true,
  );
  assert.equal(
    (await ipcMain.handlers.get("netcatty:appLock:getSystemUnlockStatus")())
      .label,
    "Touch ID",
  );
});
...
test("incorrect password attempts back off and a successful unlock resets the delay", async () => {
  ...
  assert.deepEqual(await controller.requestUnlock("wrong-1"), {
    ok: false,
    error: "incorrect",
  });
  assert.deepEqual(await controller.requestUnlock("wrong-2"), {
    ok: false,
    error: "incorrect",
  });
  ...
  controller.setLocked("manual");
  assert.deepEqual(await controller.requestUnlock("wrong-3"), {
    ok: false,
    error: "incorrect",
  });
...
test("parallel password attempts share one bounded in-flight verification", async () => {
  ...
  assert.deepEqual(
    delays,
    [250],
    "second attempt must not start another backoff",
  );
...
test("unlocked requests cannot queue ahead of a later legitimate unlock", async () => {
  const { controller, runtimeBridge, unlockFailureDelays } =
    await createControllerHarness();
  await controller.requestPasswordChange({ nextPassword: "alpha" });
  await controller.requestUnlock("alpha");

  const staleCalls = Array.from({ length: 20 }, () =>
    controller.requestUnlock("garbage"),
  );
  controller.setLocked("background");
  assert.deepEqual(await controller.requestUnlock("alpha"), { ok: true });
  assert.equal(runtimeBridge.getState().locked, false);
...
test("stale renderer cannot overwrite the latest verifier with a whole-object settings write", async () => {
  ...
  assert.notEqual(
    freshSnapshot.passwordVerifier?.hash,
    staleSnapshot.passwordVerifier?.hash,
  );

  assert.deepEqual(await controller.requestDisable("alpha"), {
    ok: false,
    error: "incorrect",
  });

  assert.deepEqual(await controller.requestDisable("bravo"), {
    enabled: false,
...
test("runtime state broadcast fans out to all main windows, session app-content windows, settings, and every popup window", async () => {
  ...
  for (const win of windows) {
    const runtimeMessages = win.sent.filter(
      ([channel]) => channel === "netcatty:appLock:runtimeStateChanged",
    );
    assert.equal(
      runtimeMessages.length,
      1,
      `${win.name} should receive one runtime broadcast`,
    );
...
test("idle lock closes DevTools in every app window", async () => {
  await withPatchedTimers(async ({ flushDueTimers }) => {
    await withPatchedDateNow(1000, async ({ setNow }) => {
      const { controller, runtimeBridge, windows } =
        await createControllerHarness();
      ...
      windows.forEach((win) => {
        assert.equal(
          win.getDevToolsCloseCount(),
          1,
          `${win.name} DevTools should close on idle lock`,
        );
      });
    });
  });
...
test("lock redacts window titles and unlock restores them", async () => {
  ...
  controller.setLocked("manual");
  assert.deepEqual(
    windows.map((win) => win.getTitle()),
    windows.map(() => "Netcatty"),
  );
...
test("resetting app lock requires the current password before clearing the verifier", async () => {
  ...
  assert.deepEqual(await controller.requestReset(), {
    ok: false,
    error: "empty-current",
  });
  assert.deepEqual(await controller.requestReset("wrong"), {
    ok: false,
    error: "incorrect",
  });
...
test("resetting app lock clears the verifier, unlocks runtime, and broadcasts settings and runtime", async () => {
  ...
  assert.equal(runtimeBridge.getState().reason, null);
  for (const win of windows) {
    const settingsMessages = win.sent.filter(
      ([channel]) => channel === "netcatty:appLock:settingsChanged",
    );
    const runtimeMessages = win.sent.filter(
      ([channel]) => channel === "netcatty:appLock:runtimeStateChanged",
    );
    assert.equal(
      ...
      1,
      `${win.name} should receive one settings broadcast`,
    );
    assert.equal(
      runtimeMessages.length,
      1,
