...
test("main/settings call sites wire Windows chrome helpers", () => {
  ...
  const popup = readFileSync(
    path.join(here, "terminalPopupWindow.cjs"),
    "utf8",
  );
  const css = readFileSync(path.join(here, "../../../index.css"), "utf8");
  const html = readFileSync(path.join(here, "../../../index.html"), "utf8");
  const helper = readFileSync(
    path.join(here, "windowsWindowChrome.cjs"),
    "utf8",
  );
  ...
  for (const [label, source] of [
    ...
    assert.match(
      source,
      /require\("\.\/windowsWindowChrome\.cjs"\)/,
      `${label} must require chrome helpers`,
    );
    assert.match(
      source,
      /windowsFramelessContentChromeOptions/,
      `${label} must use content chrome helper`,
    );
    assert.doesNotMatch(
      source,
      /resolveFramelessHostBackgroundColor/,
      `${label} should not clear host backdrop`,
    );
  ...
  assert.doesNotMatch(
    helper,
    /[^\x00-\x7F]/,
    "helper comments must stay ASCII-only",
  );
