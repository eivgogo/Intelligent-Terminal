...
function cloneState(state) {
  return {
    ...
    lastLockedAt:
      typeof state.lastLockedAt === "number" ? state.lastLockedAt : null,
    lastUnlockedAt:
      typeof state.lastUnlockedAt === "number" ? state.lastUnlockedAt : null,
    lastActivityAt:
      typeof state.lastActivityAt === "number" ? state.lastActivityAt : null,
  };
}
...
function createAppLockRuntimeBridge() {
  ...
  function initialize(nextState) {
    ...
    return applyStatePatch({
      ...
      lastLockedAt: locked
        ? typeof nextState?.lastLockedAt === "number"
          ? nextState.lastLockedAt
          : now
        : null,
      lastUnlockedAt: locked
        ? null
        : typeof nextState?.lastUnlockedAt === "number"
          ? nextState.lastUnlockedAt
          : null,
      lastActivityAt:
        typeof nextState?.lastActivityAt === "number"
          ? nextState.lastActivityAt
          : now,
    });
  }
...
function createAppLockController({
  ...
  waitForUnlockFailureDelay = (delayMs) =>
    new Promise((resolve) => setTimeout(resolve, delayMs)),
}) {
  if (
    !settingsStore ||
    typeof settingsStore.getSnapshot !== "function" ||
    typeof settingsStore.save !== "function"
  ) {
  ...
  function getWindowsForBroadcast() {
    const windows = [
      ...
      getSettingsWindow(),
      ...(Array.isArray(getTerminalPopupWindows())
        ? getTerminalPopupWindows()
        : []),
    ];
  ...
  function enforceWindowProtection(win) {
    if (!win || win.isDestroyed?.() || getRuntimeState()?.locked !== true)
      return;
    try {
  ...
  function protectWindow(win) {
    ...
    if (!protectedWindows.has(win)) {
      ...
      try {
        win.on?.("system-context-menu", suppressSystemContextMenu);
      } catch {
        /* ignore */
      }
      try {
        win.webContents?.on?.("devtools-opened", enforce);
      } catch {
        /* ignore */
      }
      try {
  ...
  function setWindowTitle(win, title) {
    ...
    const nextTitle =
      typeof title === "string" && title.trim() ? title.trim() : "Netcatty";
    try {
  ...
  async function requestDisable(currentPassword) {
    ...
    const saved = await mutateSettings(async (current) => {
      if (current.passwordVerifier) {
        ...
        const verified = await verifyAppLockPassword(
          currentPassword,
  ...
  async function requestPasswordChange(input = {}) {
    const nextPassword =
      typeof input.nextPassword === "string" ? input.nextPassword : "";
    ...
    const saved = await mutateSettings(async (current) => {
      ...
      const passwordVerifier =
        await createAppLockPasswordVerifier(nextPassword);
      return {
        ...current,
    ...
    if (enablingFromNoVerifier) {
      try {
        runtimeBridge.recordActivity(Date.now());
      } catch {
        /* ignore */
      }
      syncIdleTimer();
  ...
  async function setSystemUnlockEnabled(input = {}) {
    ...
    if (
      !systemAuthBridge ||
      typeof systemAuthBridge.requestUnlock !== "function"
    ) {
  ...
  async function requestUnlockAttempt(password, requestContext) {
    ...
    if (
      ...
      !isSamePasswordVerifier(
        requestContext.passwordVerifier,
        current.passwordVerifier,
      )
    ) {
  ...
  function registerHandlers(ipcMain) {
    ...
    ipcMain.handle(
      "netcatty:appLock:setTimeoutMinutes",
      (_event, timeoutMinutes) => setTimeoutMinutes(timeoutMinutes),
    );
    ipcMain.handle("netcatty:appLock:requestEnable", () => requestEnable());
    ipcMain.handle(
      "netcatty:appLock:requestDisable",
      (_event, currentPassword) => requestDisable(currentPassword),
    );
    ...
    ipcMain.handle("netcatty:appLock:setLocked", (_event, reason) =>
      setLocked(reason),
    );
