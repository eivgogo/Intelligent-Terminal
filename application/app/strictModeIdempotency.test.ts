...
test("every renderer root mounts under StrictMode", () => {
  ...
  assert.equal(
    renderCalls.length,
    3,
    "main, settings and terminal-popup roots",
  );
  ...
  for (let index = 0; index < renderCalls.length; index += 1) {
    ...
    assert.match(
      opener,
      /root\.render\(\s*<StrictMode>/,
      `root.render #${index + 1} lacks StrictMode`,
    );
...
test("clone-session payload is consumed once even if the effect re-runs", () => {
  const effectStart = appSideEffectsSource.indexOf(
    "consumedNewWindowSessionRef",
  );
  assert.notEqual(
    effectStart,
    -1,
    "clone-session effect must latch on payload identity",
  );
...
test("update-available toast latches on the release version", () => {
  ...
  const guardAt = startupEffectsSource.indexOf(
    "if (toastedUpdateVersionRef.current === version) return;",
  );
...
test("port-forward auto-start runs once across a StrictMode double effect", () => {
  ...
  const effectStart = portForwardingAutoStartSource.indexOf(
    "if (autoStartExecutedRef.current) return;",
  );
  assert.notEqual(
    effectStart,
    -1,
    "the effect must bail out when the latch is already set",
  );
  ...
  assert.notEqual(
    effectEnd,
    -1,
    "auto-start effect dep list moved; update this contract",
  );
  ...
  assert.ok(
    latchAt < runAt,
    "the latch must be set before runAutoStart() is called",
  );
...
test("cancelled startup update check resets its latch instead of skipping forever", () => {
  ...
  assert.match(
    tail,
    /startupCheckTimeoutRef\.current = setTimeout\(async \(\) => \{\s*\n\s*checkArmed = false;/,
  );
  assert.match(
    tail,
...
test("global hotkey registration cleans up across StrictMode remount", () => {
  ...
  const hotkeyAt = systemEffectsSource.indexOf(
...
test("settings persistMountedRef resets on StrictMode cleanup", () => {
  ...
  const markAt = settingsSource.indexOf(
    "Mark persist effects mounted AFTER all persist useEffects",
  );
...
test("terminal selection Ask-AI payload is consumed once under StrictMode", () => {
  ...
  const latchAt = hostSource.indexOf(
  ...
  assert.ok(
    latchAt > 0 && draftAt > latchAt,
    "must latch before mutating the draft",
  );
});
