...
export const zhCNTerminalMessages: Messages = {
  ...
  "terminal.networkDevice.tip.message":
    "此主机疑似为网络设备。启用网络设备模式后，命令将原样发送（不做 Shell 包装）。",
  ...
  "terminal.osc7Setup.desc":
    "Intelligent Terminal 会为当前远端用户添加 OSC 7 提示符配置。这样在 sudo 或 su 之后，SFTP 也能继续跟随终端目录。",
  ...
  "terminal.progress.autoReconnectScheduled":
    "连接已断开，将在 {seconds} 秒后自动重连（第 {attempt} 次）。",
  ...
  "terminal.et.proxyUnsupported":
    "EternalTerminal 目前不支持 Intelligent Terminal 的代理设置。请改用 SSH，或移除该主机的代理。",
  "terminal.et.multiJumpUnsupported":
    "EternalTerminal 目前在 Intelligent Terminal 中最多支持一个跳板机。",
  ...
  "settings.sftp.transferConcurrency.desc":
    "同一目标同时传输的文件数量上限，包括文件夹内文件和单独多选的文件。较高的值可能提高速度，但可能导致某些服务器过载。",
  "settings.sftp.transferPoolIdleTtl": "传输连接保活时间",
  "settings.sftp.transferPoolIdleTtl.desc":
    "专用传输 SFTP 连接空闲多久后关闭。时间更长可减少下次上传/下载的冷启动等待；进行中的传输不会被关闭。",
  ...
  "settings.ssh.transportIdleTtl.desc":
    "最后一个终端、SFTP 浏览、批量传输或端口转发释放后，共享 SSH 连接再保留多久。保留期间再次打开同主机可跳过重新认证（含 MFA）；进行中的会话与传输不会被关闭。",
  ...
  "settings.sftp.showHiddenFiles.desc":
    "在 SFTP 文件浏览器中显示隐藏文件（Unix/macOS 点文件和 Windows 隐藏属性文件）。",
  ...
  "settings.sftp.compressedUpload.enableDesc":
    "自动使用 tar 压缩文件夹后再传输。需要服务器支持 tar 命令，不支持时自动回退到普通传输。",

  "settings.sftp.skipUnchanged.enable": "跳过未改文件",
  "settings.sftp.skipUnchanged.enableDesc":
    "当目标文件的大小与修改时间已与源一致时跳过传输（类似 rsync）。替换到暂存路径时自动禁用。",
  ...
  "settings.terminal.font.cjk.alignmentWarning":
    "这个字体尚未确认是等宽字体，请通过预览检查文字是否对齐。",
  "settings.terminal.font.cjk.unavailableWarning":
    "本机没有安装这个字体。Intelligent Terminal 会保留设置，并使用安全字体继续显示。",
  ...
  "settings.terminal.behavior.autoCloseOnExit.desc":
    "允许终端标签页和窗口在会话退出后自动关闭。关闭此项后，无论退出结果如何都会保留。",
  ...
  "settings.terminal.behavior.copyOnSelect.desc":
    "自动复制选中的文本。在 tmux/vim 鼠标模式下，macOS 按住 Option，Windows/Linux 按住 Shift 拖选即可选中文本",
  ...
  "settings.terminal.startupCommandDelay.desc":
    "连接建立后等待多久再发送启动命令；启动命令为多行时，行与行之间也使用该间隔。慢连接可调大。",
  ...
  "settings.terminal.localShell.shell.desc":
    "Shell 可执行文件的路径（例如 /bin/zsh、pwsh.exe）。留空使用系统默认。",
  ...
  "settings.terminal.localShell.shell.isDirectory":
    "路径是目录，不是可执行文件",
  ...
  "settings.terminal.localShell.shell.customArgs.desc":
    "传给 Shell 的启动参数。部分 Shell 必须指定才能正常工作,例如 msys2 bash 需要 --login -i 才能加载环境变量。",
  ...
  "settings.terminal.serverStats.show.desc":
    "在终端状态栏显示 CPU、内存和磁盘使用情况（仅限 Linux 服务器）。",
  ...
  "settings.terminal.rendering.renderer.desc":
    "选择终端渲染技术。自动模式会在低内存设备上使用 DOM 渲染。更改将在新终端会话中生效。",
  ...
  "settings.terminal.inlineImages.hibernateNote":
    "已绘制内联图片的标签页会保留渲染器而不进入完整休眠，因为休眠快照只能恢复文本。",
